...
class CuentaBancaria:
    def __init__(self, nombres_titular, apellidos_titular, numero_cuenta, tipo_cuenta):
        ...
        self.saldo = 0 #Inicializa el saldo de la cuenta en 0 al momento de su creación.

    def imprimir(self):
        print(f"Nombres del titular: {self.nombres_titular}")
        print(f"Apellidos del titular: {self.apellidos_titular}")
        print(f"Número de cuenta: {self.numero_cuenta}")
        print(f"Tipo de cuenta: {self.tipo_cuenta.name}") # Accedemos al nombre del enum
...
if __name__ == "__main__": #Corre en el caso de que el script se ejecute directamente
